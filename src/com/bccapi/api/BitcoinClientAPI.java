...
public interface BitcoinClientAPI {
   ...
   /**
    * Get a login challenge from the BccServer for an account identified by its
    * public key. The challenge is used with the login function, and is valid
    * for a limited time.
    * 
   ...
   /**
    * Login with the public key of an account and a signed response of the
    * challenge obtained from getLoginChallenge. If this is the first successful
    * login with an account public key a new account is created. If the account
    * is left idle for a significant amount of time and its associated wallet
    * left empty, then the account may get deleted.
    * 
    * @param accountPublicKey
    *           The public key of the account to login with.
    * @param challengeResponse
    *           The challenge obtained from a call to getLoginChallenge signed
    *           with the account private key.
    * @return A session ID providing access to other functions in the Bitcoin
    *         Client API.
    * @throws IOException
    *            If communication with server fails
    * @throws APIException
   ...
   /**
    * Get a {@link SendCoinForm} for sending bitcoins to a bitcoin address. The
    * form returned contains a transaction that needs to be signed and submitted
    * with {@link BitcoinClientAPI#submitTransaction}. The transaction needs to
    * be signed with the private keys corresponding to the public keys that have
    * been added to the wallet using {@link BitcoinClientAPI#addKeyToWallet}.
    * The {@link SendCoinForm} contains the indexes of each public keys that
    * needs to sign the various inputs of the transaction. Those indexes refer
    * to the private keys in the order they were added to the server.
    * 
    * @param sessionID
    *           The session ID.
   ...
   /**
    * Submit a signed transaction. The server will validate that the transaction
    * has been signed appropriately and is valid according to available funds,
