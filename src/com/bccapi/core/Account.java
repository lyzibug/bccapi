...
public class Account {
	...
	public synchronized void addKey() throws APIException, IOException {
		AccountInfo info = _api.getAccountInfo(_sessionId);
		byte[] publicKeyBytes = getWalletPublicKey(info.getKeys())
				.getPubKeyBytes();
	 ...
	 *            0.000000001 BTC)
	 * @param fee
	 *            The size of the fee to include in the transaction in satoshis
	 *            (1 satoshi = 0.000000001 BTC)
	 * @return A {@link SendCoinForm}
	 * @throws APIException
	 * @throws IOException
	 */
	public synchronized SendCoinForm getSendCoinForm(String receivingAddress,
			long amount, long fee) throws APIException, IOException {
		SendCoinForm form = _api.getSendCoinForm(_sessionId, receivingAddress,
				amount, fee);
		return form;
	}
	 ...
	 * @throws IOException
	 */
	public void signAndSubmitSendCoinForm(SendCoinForm form)
	...
	public List<String> getAddresses() {
		...
		for (PublicECKey key : _keyManager.getPublicKeys()) {
			...
			addresses.add(AddressUtil.publicKeyToStringAddress(
					_api.getNetwork(), key.getPubKeyBytes()));
		}
		return addresses;
	}
	...
	private byte[] calculateLoginChallengeResponse(byte[] challenge) {
		byte[] accountPublicKeyBytes = _keyManager.getPublicKey(0)
				.getPubKeyBytes();
	...
	private static void signSendCoinForm(SendCoinForm form, Account account) {
		...
		for (int i = 0; i < inputs.size(); i++) {
			...
			if (signer == null) {
				throw new RuntimeException(
						"Unable to find signer for key with index: " + index);
			}
		...
		for (int i = 0; i < inputs.size(); i++) {
			...
			BccScriptInput script = new BccScriptInput(signatures[i],
					key.getPubKeyBytes());
